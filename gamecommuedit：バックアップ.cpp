	...
	else
	{
		...
		if (IsLEFTKeyTrigger(key, 0) == TRUE)
		{
			...
			case EDIT_CHARACTER_RIGHT:
				...
				break;
			case EDIT_BACKGROUND:	if (edit_row_background > 0) { edit_row_background--; }	break;
...
#pragma endregion
			case EDIT_BACKGROUND:	if (edit_row_background < (bgimage[0].GetNumber() - 1) / 10 + 1) { edit_row_background++; }	break;
			case EDIT_BGM:			if (edit_row_bgm < 3) { edit_row_bgm++; }												break;
...
#pragma endregion
		...
		case EDIT_BGM:
			...
			else { sprintf_s(c, 256, "ページ%d/4\n←→キー：ページ切り替え\n0：%s\n1：%s\n2：%s\n3：%s\n4：%s\n5：%s\n6：%s\n7：%s\n8：%s\n9：%s\nB：キャンセル\nD：削除\n", edit_row_bgm + 1, c_x[0], c_x[1], c_x[2], c_x[3], c_x[4], c_x[5], c_x[6], c_x[7], c_x[8], c_x[9]); }
			break;
		case EDIT_SE:
			// 10個の変数を代入
			for (int i = 0; i < 10; i++)
